...
def send_email():
    ...
    file_path = 'yj_house_' + str(time.strftime('%Y-%m-%d', time.localtime(time.time()))) + '.xlsx'
    f = open(file_path, 'rb')
    ...
    # file_name = 'yj_house_' + str(time.strftime('%Y-%m-%d', time.localtime(time.time())))
    file_path2 = 'yj_house_day_' + str(time.strftime('%Y-%m-%d', time.localtime(time.time()))) + '.xlsx'
    ...
    # file_name = 'yj_house_' + str(time.strftime('%Y-%m-%d', time.localtime(time.time())))
    file_path3 = 'yj_house_week_' + str(time.strftime('%Y-%m-%d', time.localtime(time.time()))) + '.xlsx'
...
def db_save_excel_days():
    ...
    annex_path = './yj_house_' + data_date_str + '.xlsx'
...
def db_save_excel_calc_day():
    data_date_str = str(time.strftime('%Y-%m-%d',time.localtime(time.time())))
    annex_path = './yj_house_day_' + data_date_str + '.xlsx'
    sql = """
    select
        project_name
        ,project_distinct
    ...
    # columns = [x[0] for x in col_result]
    # 定义表头
    columns = ['项目名称','所在地区','总套数','已售住宅套数','住宅均价','已售非住宅套数','非住宅均价','数据批次时间']
    data = pd.DataFrame(result, columns=columns)
    print('日采集落excel数据量：',len(data))
    data.to_excel(annex_path, index=False)  # 将数据库数据保存到excel中
...
def db_save_excel_calc_week():
    data_date_str = str(time.strftime('%Y-%m-%d',time.localtime(time.time())))
    annex_path = './yj_house_week_' + data_date_str + '.xlsx'
    sql = """
    select
        project_name
        ,project_distinct
    ...
    # columns = [x[0] for x in col_result]
    # 定义表头
    columns = ['项目名称','所在地区','总套数','本周已售住宅套数','住宅均价','本周已售非住宅套数','非住宅均价','数据批次时间']
    data = pd.DataFrame(result, columns=columns)
    print('日采集落excel数据量：',len(data))
    data.to_excel(annex_path, index=False)  # 将数据库数据保存到excel中
