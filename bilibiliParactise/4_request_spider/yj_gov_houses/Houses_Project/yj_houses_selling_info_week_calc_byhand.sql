...
-- 删除当天数据
DELETE FROM yj_houses.yj_houses_selling_info_week WHERE data_date = '2022-03-01';
-- 插入数据表
...
select 
    ...
    ,total_house_num
    ,sold_house_num
    ,week_sold_house_num + 0 as week_sold_house_num
    ,house_avg_price
    ,sold_not_house_num
    ,week_sold_not_house_num + 0 as week_sold_not_house_num
    ,not_house_avg_price
    ,data_date
from (
SELECT
...
FROM (select * from yj_houses.yj_houses_selling_info WHERE data_date = '2022-03-01' AND (
...
left join 
    (select project_name,sold_house_num,sold_not_house_num from yj_houses.yj_houses_selling_info WHERE data_date = '2022-02-28') b 
