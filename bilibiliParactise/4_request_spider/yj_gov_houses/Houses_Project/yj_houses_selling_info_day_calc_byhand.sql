...
-- 删除当天数据
DELETE FROM yj_houses.yj_houses_selling_info_day WHERE data_date = '2022-03-01';
-- 插入数据表
...
(SELECT DISTINCT 
                ...
                ,data_date
            FROM yj_houses.yj_houses_selling_info WHERE data_date = '2022-03-01'
            AND (
...
(
    SELECT DISTINCT 
                ...
                ,not_house_avg_price  -- 非住宅均价
                ,data_date
            FROM yj_houses.yj_houses_selling_info WHERE data_date = '2022-02-28'
            AND (
            project_name LIKE '%上东新城%'
    OR project_name LIKE '%保利中央公园%'
    OR project_name LIKE '%恒隆共青湖%'
