...
DROP PROCEDURE IF EXISTS `yj_houses_selling_info_day_calc`$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `yj_houses_selling_info_day_calc`()
BEGIN
-- 删除当天数据
DELETE FROM yj_houses_selling_info_day WHERE data_date = CURRENT_DATE();
-- 插入数据表
...
(SELECT DISTINCT 
                ...
                ,data_date
            FROM yj_houses.yj_houses_selling_info WHERE data_date = CURRENT_DATE()
            AND (
...
(
    SELECT DISTINCT 
                ...
                ,not_house_avg_price  -- 非住宅均价
                ,data_date
            FROM yj_houses.yj_houses_selling_info WHERE data_date = DATE_SUB(CURRENT_DATE(),INTERVAL + 1 DAY)
            AND (
            project_name LIKE '%上东新城%'
    OR project_name LIKE '%保利中央公园%'
    OR project_name LIKE '%恒隆共青湖%'
