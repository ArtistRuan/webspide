...
BEGIN
-- 删除当天数据
DELETE FROM yj_houses.yj_houses_selling_info_week WHERE data_date = CURRENT_DATE();
-- 插入数据表
...
FROM yj_houses.yj_houses_selling_info_day WHERE YEARWEEK(DATE_FORMAT(data_date,'%Y-%m-%d'),1) = YEARWEEK(NOW(),1)
