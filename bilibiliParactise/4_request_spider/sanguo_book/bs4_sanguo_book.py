...
'''
@title: bs4_sanguo_book
@projectName pythonProject
@description: 获取 https://www.shicimingju.com/book/sanguoyanyi.html 下《三国演义》的所有章节和内容，保存到文本文件
@author ruanshikao
...
def get_book():
    ...
    page_text = requests.get(url=url,headers=headers)
    page_text.encoding = 'utf-8'  # 页面编码为 <meta charset="UTF-8">
    # 保存到文件
    page_html = './sanguoyanyi.html'
    with open(page_html,'w',encoding='utf-8') as fp:
    ...
    for li in li_list:
        ...
        detail_page_text.encoding='uft-8'
        # 不能用已经用过的soup，所以要实例化新的soup
        soup_detail = BeautifulSoup(detail_page_text.text,'lxml')
        # detail_div_tag = soup_detail.select('.chapter_content')  # 这里报错，要分析原因，看看select怎么用
        # 获取详情页的章节内容
        detail_div_tag = soup_detail.find('div',class_='chapter_content')
        # 解析章节内容为文本
        detail_content = detail_div_tag.text
